...
const EquivalenceFormPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            allKeys.forEach(key => {
                if (key.startsWith('temp_sending_equivalence_') && !key.includes(client.id)) {
                    localStorage.removeItem(key);
                }
            });
        }

        if (isSendingTemporarily && client?.isSendingFormulaireEquivalence) {
    ...
    }, [client?.id, client?.isSendingFormulaireEquivalence, client?.equivalenceStatus, isSendingTemporarily]);
    ...
    const canResubmit = client?.equivalenceStatus !== 'rejected' ||
            ...
            <div className="py-12 sm:py-16 md:py-20 bg-slate-50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="text-center mb-8 sm:mb-12">
                        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-blue-900">🎓 Formulaire Équivalence de diplôme</h1>
                        <p className="text-base sm:text-lg text-slate-600 mt-2">Fournissez les informations nécessaires pour la demande d&apos;équivalence.</p>
                    </div>

                    <FormSection id="equivalence" title="" subtitle="">
                        {(client?.isSendingFormulaireEquivalence && !canResubmit) || isSendingTemporarily ? (
                            <FormStatusDisplay
                                status={isSendingTemporarily ? 'pending' : (client?.equivalenceStatus || null)}
                            ...
                            <>
                                {client?.equivalenceStatus === 'rejected' && canResubmit && <ResubmissionWarning />}
                                <FormulaireEquivalence onSubmitSuccess={handleSubmit} />
                            </>
                        )}
                    </FormSection>
                </div>
